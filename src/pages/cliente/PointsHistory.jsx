...
export const PointsHistory = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const [txRes, pointsRes] = await Promise.all([
                ...
                supabase.from('points_accounts')
                    .select('current_points, lifetime_points, tier_level')
                    .eq('user_id', user.id)
                    .maybeSingle()
            ]);
    ...
    return (
        <div className="p-4">
            {/* Balance Card */}
            <motion.div className="bg-gradient-to-br from-secondary to-cyan-400 rounded-2xl p-6 text-white mb-6 shadow-lg"
                initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                <div className="flex items-center justify-between mb-2">
                    <span className="text-sm opacity-80">Tu Balance</span>
                    <span className="text-lg">{TIER_EMOJI[balance.tier]} {balance.tier.charAt(0).toUpperCase() + balance.tier.slice(1)}</span>
                </div>
                <p className="text-4xl font-bold mb-1">{balance.current.toLocaleString()} pts</p>
                <p className="text-xs opacity-70">Puntos de por vida: {balance.lifetime.toLocaleString()}</p>
            </motion.div>
            ...
            {filtered.length === 0 ? (
                <div className="text-center py-12">
                    <Star className="w-12 h-12 text-gray-medium mx-auto mb-3 opacity-30" />
                    <p className="text-gray-medium">Sin movimientos</p>
                </div>
            ) : (
                <div className="space-y-2">
                    {filtered.map((tx, i) => {
                        ...
                        return (
                            <motion.div key={tx.id} className="bg-white rounded-xl shadow-sm p-4 flex items-center gap-3"
                                initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}>
                                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${config.bg}`}>
                                    <Icon className={`w-5 h-5 ${config.color}`} />
                                </div>
                                <div className="flex-1 min-w-0">
                                    <p className="text-sm font-medium text-gray-dark truncate">{tx.description || config.label}</p>
                                    <p className="text-[11px] text-gray-medium">{format(new Date(tx.created_at), "d 'de' MMM, HH:mm", { locale: es })}</p>
                                </div>
                                <span className={`font-bold text-sm ${config.color}`}>
                                    {config.sign}{Math.abs(tx.points_amount)} pts
                                </span>
                            </motion.div>
                        );
                    })}
