...
export const UploadFiles = () => {
    ...
    // Calculate price for a specific size and quality based on selected location
    const getUnitPrice = (sizeId, q = 'standard') => {
        const loc = locations.find(l => l.id === selectedLocation);
        let priceKey = '';
        if (sizeId.startsWith('foto')) {
            priceKey = `foto_${sizeId}`; // e.g. foto_10x15
        } else {
            priceKey = `${sizeId}_${q === 'premium' ? 'high' : 'eco'}`; // e.g. a4_eco
        }

        // Check if local has custom override
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const { data: order, error } = await supabase.from('print_orders').insert({
                customer_id: user.id, location_id: selectedLocation, file_urls: uploadedFiles,
                specifications: { size: selectedSize, quality, copies, color: quality === 'premium' && selectedLocObj?.has_color_printing },
                total_amount: total, points_earned: pts, status: 'pendiente',
    ...
    // Filter available sizes based on location capabilities
                    ...
                    <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                        ...
                        <div className="flex gap-3 mt-8">
                            ...
                            <button disabled={!selectedLocation} onClick={() => {
                                // If size is currently selected but no longer available (e.g. was photo, new location doesn't have fotoya), reset it.
                    ...
                    <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
                                ...
                                {availableSizes.length === 0 && <p className="text-sm text-secondary bg-red-50 p-3 rounded-lg mt-3">No hay tamaños disponibles en este local.</p>}
                            </div>

                            {/* Calidad solo si es documento */}
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        ...
                                        {canPrintColor ? (
                                            ...
                                            <div className="p-4 rounded-xl border-2 border-dashed border-gray-200 text-left opacity-60 flex flex-col justify-center">
                                                ...
                                                <div className="text-xs text-gray-400 mt-1">No disponible en este local</div>
                                            </div>
                                        )}
                                    </div>
                                </div>
                            ...
                            <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
                                <div>
                                    ...
                                    <span className="text-sm text-gray-medium">De cada archivo subido</span>
                                </div>
                ...
                {step === 4 && (
                    <motion.div key="step4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                        ...
                        <div className="mb-6 bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
                            <label className="block text-gray-dark font-bold mb-2">Instrucciones especiales para el local (opcional)</label>
                            <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Ej: Imprimir en alta calidad, sin márgenes, anillar si es posible..."
                        ...
                        <label className="flex items-start mb-8 cursor-pointer p-4 bg-primary/5 rounded-xl border border-primary/20 hover:bg-primary/10 transition-colors">
                            ...
                            <span className="text-sm text-gray-dark font-medium leading-tight">He verificado que mis archivos están correctos y acepto el compromiso de pago al momento de retirar mis impresiones en el local seleccionado.</span>
                        </label>
                        ...
                        <div className="flex gap-3">
                            ...
                            <button disabled={!acceptedTerms || isSubmitting} onClick={handleSubmit}
                                className="flex-1 bg-primary text-white py-4 rounded-xl font-bold disabled:opacity-50 flex items-center justify-center gap-2 shadow-brand text-lg">
                                {isSubmitting ? <><Loader className="w-5 h-5 animate-spin" /> Procesando Pedido...</> : <><CheckCircle className="w-5 h-5" /> Confirmar Pedido</>}
                            </button>
                        </div>
                    </motion.div>
                )}
                ...
                {step === 5 && orderResult && (
                    <motion.div key="step5" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-12">
                        ...
                        <div className="flex flex-col sm:flex-row gap-3 max-w-sm mx-auto">
                            <button onClick={() => navigate('/cliente/orders')} className="flex-1 bg-primary text-white py-4 rounded-xl font-bold shadow-brand hover:bg-primary/90 transition text-lg">Mis Órdenes</button>
                            <button onClick={() => navigate('/cliente/dashboard')} className="flex-1 bg-gray-100 text-gray-dark py-4 rounded-xl font-bold hover:bg-gray-200 transition text-lg">Volver al Inicio</button>
                        </div>
                    </motion.div>
                )}
            </AnimatePresence>
