    ...
    return (
        <div className="p-4 space-y-6">
            ...
            >
                ...
                <Link to="/cliente/points-history" className="block text-center mt-4">
            ...
            <div className="grid grid-cols-2 gap-4">
                <Link to="/cliente/upload">
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
                        whileHover={{ scale: 1.03, boxShadow: '0 10px 25px -5px rgba(235, 28, 36, 0.4)' }} whileTap={{ scale: 0.97 }}
                ...
                <Link to="/cliente/rewards">
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
                        whileHover={{ scale: 1.03, boxShadow: '0 10px 25px -5px rgba(164, 204, 57, 0.4)' }} whileTap={{ scale: 0.97 }}
            ...
            <div className="bg-white rounded-xl shadow-lg p-4">
                <div className="flex items-center justify-between mb-4">
                    ...
                    <Link to="/cliente/orders" className="text-primary text-sm font-medium flex items-center gap-1">
                        ...
                        {locations.map(loc => {
                            ...
                            return (
                                <motion.div key={loc.id} className={`p-4 rounded-xl border ${actuallyOpen ? 'bg-success/5 border-success/20' : 'bg-gray-50 border-gray-100'} flex justify-between items-center transition-colors`}
                                    initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 * arguments[1] }}
                                    ...
                                    <div className="flex items-center flex-col gap-1 items-end min-w-[70px]">
                                        ...
                                        {!actuallyOpen && isOnline && !loc.is_open && (
                                            <span className="text-[9px] text-gray-400">Pausado</span>
                                        )}
                                        {!actuallyOpen && !isOnline && (
                                            <span className="text-[9px] text-gray-400">Desconectado</span>
                                        )}
                                    </div>
                                </motion.div>
                            );
            ...
            <Link to="/cliente/upload">
                <motion.button className="fixed bottom-20 right-4 z-40 w-14 h-14 bg-primary text-white rounded-full shadow-brand-lg flex items-center justify-center"
